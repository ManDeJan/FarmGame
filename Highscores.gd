...
func post_highscore(name, highscore):
	...
	if OS.is_debug_build():
		error = $HTTPRequest.request("http://127.0.0.1:5000/highscores", headers, false, HTTPClient.METHOD_POST, query_string)
	else:
		error = $HTTPRequest.request("JAN VUL DIT IN", headers, true, HTTPClient.METHOD_POST, query_string) #bool is voor https
	if error != OK:
